...
const solutions = [
  {
    ...
    metrics: [["RESPONSE FLOW", "CONNECTED"], ["HUMAN GATE", "ACTIVE"], ["TRACE", "COMPLETE"]],
  ...
  },
  {
    ...
    metrics: [["CHANNELS", "CONNECTED"], ["CONFIDENCE", "VISIBLE"], ["HANDOFF", "CONTROLLED"]],
  ...
  },
  {
    ...
    productHref: "/ai-operations",
    productLabel: "AI Operations entdecken",
    ...
    metrics: [["WORKFLOW", "ORCHESTRATED"], ["EXCEPTIONS", "VISIBLE"], ["SYSTEM WRITE", "CONTROLLED"]],
    nodeA: "ERP / DOC / EVENT",
    nodeB: "OPS AGENT",
  ...
  },
  {
    ...
    metrics: [["SOURCES", "GROUNDED"], ["ACCESS", "ROLE BASED"], ["EVIDENCE", "VISIBLE"]],
...
export function SolutionsHubExperience() {
  ...
  const active = useMemo(() => solutions.find((solution) => solution.id === activeId) ?? solutions[0], [activeId]);

  useEffect(() => {
    if (!isRunning) return;
    const timer = window.setInterval(() => setRunIndex((current) => (current + 1) % active.stages.length), 1750);
    ...
    const setup = async () => {
      ...
      const context = gsap.context(() => {
        root.querySelectorAll<HTMLElement>("[data-sol-reveal]").forEach((element) => {
          gsap.fromTo(element, { y: 30, autoAlpha: 0 }, { y: 0, autoAlpha: 1, duration: .9, ease: "power3.out", scrollTrigger: { trigger: element, start: "top 86%", once: true } });
        });
        root.querySelectorAll<HTMLElement>("[data-sol-line]").forEach((line, index) => {
          gsap.fromTo(line, { xPercent: index % 2 === 0 ? -4 : 4, autoAlpha: .4 }, { xPercent: 0, autoAlpha: 1, ease: "none", scrollTrigger: { trigger: line, start: "top 92%", end: "top 56%", scrub: .6 } });
        });
      }, root);
    ...
    <div className={styles.root} ref={rootRef}>
      <section className={styles.hero} aria-labelledby="solutions-hero-title">
        ...
        <div className={styles.container}>
          <div className={styles.heroTopline}><span>CONLYRA / SOLUTIONS</span><strong>AI SYSTEMS FOR REAL WORK</strong></div>
          <div className={styles.heroLayout}>
            <div className={styles.heroCopy} data-sol-reveal>
              <p>WHERE DOES WORK SLOW DOWN?</p>
              <h1 id="solutions-hero-title">Nicht jede Aufgabe braucht AI.<span>Die richtigen schon.</span></h1>
            </div>
            <div className={styles.heroAside} data-sol-reveal>
              <p>CONLYRA verbindet Agenten, Workflows, Unternehmenswissen und Kontrollmechanismen dort, wo reale Arbeit messbar schneller, konsistenter und nachvollziehbarer werden kann.</p>
              <a href="#solution-system">Lösungsräume entdecken <Arrow /></a>
            </div>
          </div>
          <div className={styles.heroRail}><span>SALES</span><i /><span>SERVICE</span><i /><span>OPERATIONS</span><i /><span>KNOWLEDGE</span></div>
        </div>
      </section>

      <section className={styles.shift} aria-labelledby="solutions-shift-title">
        <div className={styles.container}>
          <div className={styles.shiftHeader} data-sol-reveal><p>THE REAL QUESTION</p><h2 id="solutions-shift-title">Wo verliert Ihr Unternehmen heute Zeit, Kontext oder Kontrolle?</h2></div>
          <div className={styles.shiftRows}>
            {solutions.map((solution) => (
              <button id={solution.id} key={solution.id} type="button" data-sol-line className={solution.id === active.id ? styles.shiftRowActive : ""} onClick={() => selectSolution(solution.id)}>
                <small>{solution.no}</small><strong>{solution.title.replace("AI für ", "")}</strong><span>{solution.statement}</span><i><Arrow /></i>
              </button>
            ))}
          </div>
        </div>
      ...
      <section className={styles.systemSection} id="solution-system" aria-labelledby="solution-system-title">
        <div className={styles.container}>
          <div className={styles.systemIntro} data-sol-reveal><p>{active.eyebrow}</p><h2 id="solution-system-title">{active.statement}</h2><span>{active.copy}</span></div>
          <div className={styles.systemShell}>
            <header className={styles.systemTopbar}>
              ...
              <nav aria-label="Lösungsbereiche">
                {solutions.map((solution) => <button key={solution.id} type="button" onClick={() => selectSolution(solution.id)} data-active={solution.id === active.id}>{solution.no} / {solution.title.replace("AI für ", "")}</button>)}
              </nav>
              <button className={styles.runControl} type="button" onClick={() => setIsRunning((current) => !current)} data-running={isRunning}><i /> {isRunning ? "LIVE RUN" : "PAUSED"}</button>
            </header>
            <div className={styles.systemBody}>
              <aside className={styles.painPanel}>
                <span>DETECTED FRICTION</span><h3>{active.title}</h3>
                <div className={styles.painList}>{active.pain.map((item, index) => <div key={item}><small>{String(index + 1).padStart(2, "0")}</small><strong>{item}</strong></div>)}</div>
              ...
              <div className={styles.executionStage}>
                ...
                <div className={styles.nodeMap}>
                  ...
                  <svg viewBox="0 0 1000 500" aria-hidden="true"><path d="M120 250 C250 250 250 120 430 120" /><path d="M120 250 C250 250 250 380 430 380" /><path d="M540 120 C650 120 650 250 780 250" /><path d="M540 380 C650 380 650 250 780 250" /></svg>
                  <span className={styles.travelPulse} aria-hidden="true" />
                </div>
                <div className={styles.pipeline}>
                  {active.stages.map((stageName, index) => {
                    const state = index < runIndex ? "complete" : index === runIndex ? "active" : "waiting";
                    return <button key={stageName} type="button" data-state={state} onClick={() => { setIsRunning(false); setRunIndex(index); }}><small>{String(index + 1).padStart(2, "0")}</small><strong>{stageName}</strong><i /></button>;
                  })}
                </div>
                <div className={styles.metricsBar}>{active.metrics.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className={styles.capabilities} aria-labelledby="solution-capabilities-title">
        <div className={styles.container}>
          <div className={styles.capabilityHeader} data-sol-reveal><p>ONE SYSTEM / MULTIPLE CAPABILITIES</p><h2 id="solution-capabilities-title">Lösungen entstehen nicht aus einem Tool.</h2><span>Sie entstehen aus dem richtigen Zusammenspiel von Intelligenz, Daten, Ausführung und Kontrolle.</span></div>
          <div className={styles.capabilityGrid}>
            {capabilityMatrix.map(([title, copy, href], index) => <Link href={href} key={title} className={styles.capabilityCard} data-sol-reveal><div><small>{String(index + 1).padStart(2, "0")}</small><span>CAPABILITY</span></div><h3>{title}</h3><p>{copy}</p><footer><span>EXPLORE SYSTEM</span><Arrow /></footer></Link>)}
          </div>
        </div>
      </section>
      ...
      <section className={styles.closing} aria-labelledby="solutions-closing-title">
        <div className={styles.container}>
          ...
          <div className={styles.closingLayout}>
            <div data-sol-reveal><p>START WITH THE WORK</p><h2 id="solutions-closing-title">Der beste AI-Startpunkt ist nicht ein Tool. Es ist ein echter Prozess.</h2></div>
            <aside data-sol-reveal><p>Wir analysieren, wo manuelle Übergaben, fehlender Kontext oder wiederkehrende Entscheidungen ein sinnvolles Automatisierungspotenzial schaffen.</p><Link href="/#contact">Workflow Audit anfragen <Arrow /></Link></aside>
          </div>
        </div>
      </section>
    </div>
