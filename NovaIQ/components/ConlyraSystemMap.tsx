  ...
  return (
    <section className={styles.root} id="system-map" ref={rootRef} aria-labelledby="conlyra-system-map-title" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocusCapture={() => setPaused(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setPaused(false); }}>
      ...
      <div className={styles.container}>
        <header className={styles.head} data-system-map-reveal><div><p>CONLYRA SYSTEM MAP / 07 OF 07 ONLINE</p><h2 id="conlyra-system-map-title">Sieben Product Worlds.<span>Ein Operating System.</span></h2></div><aside><span>COMPLETE SYSTEM ARC</span><strong>STRATEGY → CONTEXT → EXECUTION → CONTROL</strong><small>Wählen Sie einen Node, um das System zu erkunden.</small></aside></header>
        <div className={styles.systemShell} data-system-map-reveal>
          <div className={styles.mapPanel}>
            ...
            <div className={styles.mapStage}>
              ...
              <svg className={styles.paths} viewBox="0 0 1000 700" preserveAspectRatio="none" aria-hidden="true"><path d="M150 147 C270 150 300 250 500 350"/><path d="M150 518 C285 510 320 420 500 350"/><path d="M340 70 C400 160 430 220 500 350"/><path d="M660 70 C600 160 570 220 500 350"/><path d="M850 147 C730 150 700 250 500 350"/><path d="M850 518 C715 510 680 420 500 350"/><path d="M500 616 C500 530 500 455 500 350"/><path className={styles.orbitPath} d="M150 147 C330 10 670 10 850 147 C980 280 980 470 850 518 C660 700 340 700 150 518 C20 420 20 250 150 147Z"/></svg>
        ...
        <div className={styles.systemArc} data-system-map-reveal>{pathSequence.map((code,index) => { const product=products.find((item) => item.code===code)!; return <button type="button" key={product.code} data-active={active.code===product.code} onClick={() => setActiveIndex(products.findIndex((item) => item.code===product.code))}><small>{String(index+1).padStart(2,"0")}</small><span>{product.question}</span><strong>{product.name}</strong><i aria-hidden="true">→</i></button>; })}</div>
      </div>
